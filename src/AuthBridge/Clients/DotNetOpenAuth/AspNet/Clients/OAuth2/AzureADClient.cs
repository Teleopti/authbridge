	...
	public sealed class AzureADClient : OAuth2Client {
		...
		private AzureADClaims ParseAccessToken(string token, bool validate) {
			try {
				...
				AzureADClaims claimsAD = JsonConvert.DeserializeObject<AzureADClaims>(jwtClaims);
				AzureADHeader headerAD = JsonConvert.DeserializeObject<AzureADHeader>(jwtHeader);
